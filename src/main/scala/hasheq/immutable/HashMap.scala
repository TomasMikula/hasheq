...
sealed class HashMap[A, +B] extends Iterable[(A, B)]
{
  ...
  private[hasheq] def get0(key: A, hash: Int, level: Int)(implicit A: Equal[A]): Option[B] = None

  private[hasheq] def updated0[B1 >: B](key: A, hash: Int, level: Int, value: B1, kv: (A, B1), merger: Merger[A, B1])(implicit A: Equal[A]): HashMap[A, B1] =
    new HashMap.HashMap1(key, hash, value, kv)

  protected def removed0(key: A, hash: Int, level: Int)(implicit A: Equal[A]): HashMap[A, B] = this
   ...
   */
  def merged[B1 >: B](that: HashMap[A, B1])(mergef: MergeFunction[A, B1])(implicit A: Equal[A]): HashMap[A, B1] = merge0(that, 0, liftMerger(mergef))

  protected def merge0[B1 >: B](that: HashMap[A, B1], level: Int, merger: Merger[A, B1])(implicit A: Equal[A]): HashMap[A, B1] = that
...
object HashMap {
  ...
  implicit def mapReprInstance[K: HashEqual]: MapRepr[HashMap, K] = new MapRepr[HashMap, K] {
    ...
    def put[V](m: HashMap[K, V], k: K, v: V)(implicit E: Equal[K]): HashMap[K, V] = m.put(k, v)
    def size[V](m: HashMap[K, V]): Int = m.size

    override def entries[V](m: HashMap[K, V]): Iterable[(K, V)] = m
    override def updated[V](m: HashMap[K, V], k: K, v: V)(combine: (V, V) => V)(implicit E: Equal[K]): HashMap[K, V] = m.updated(k, v)(combine)
  }
  ...
  private def makeHashTrieMap[A, B](hash0:Int, elem0:HashMap[A, B], hash1:Int, elem1:HashMap[A, B], level:Int, size:Int)(implicit A: Equal[A]) : HashTrieMap[A, B] = {
  ...
  class HashMap1[A,+B](private[hasheq] val key: A, private[hasheq] val hash: Int, private[hasheq] val value: (B @uV), private[hasheq] var kv: (A,B @uV)) extends HashMap[A,B] {
    ...
    private[hasheq] override def updated0[B1 >: B](key: A, hash: Int, level: Int, value: B1, kv: (A, B1), merger: Merger[A, B1])(implicit A: Equal[A]): HashMap[A, B1] =
    ...
    override def removed0(key: A, hash: Int, level: Int)(implicit A: Equal[A]): HashMap[A, B] =
    ...
    protected override def merge0[B1 >: B](that: HashMap[A, B1], level: Int, merger: Merger[A, B1])(implicit A: Equal[A]): HashMap[A, B1] = {
  ...
  class HashTrieMap[A, +B](
    ...
    protected override def merge0[B1 >: B](that: HashMap[A, B1], level: Int, merger: Merger[A, B1])(implicit A: Equal[A]): HashMap[A, B1] = that match {
