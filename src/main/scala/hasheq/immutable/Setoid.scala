...
trait Setoid[S[A0, _ <: Equiv[A0]], A, Eq <: Equiv[A]] {
  ...
  def empty: S[A, Eq]
  def singleton(a: A): S[A, Eq] = add(empty, a)
  def fromIterable(col: Iterable[A]): S[A, Eq] = fromIterator(col.iterator)
  def fromIterator(it: Iterator[A]): S[A, Eq] = it.foldLeft(empty)(add(_, _))

  def size(s: S[A, Eq]): Int
  def isEmpty(s: S[A, Eq]): Boolean = size(s) == 0
  def contains(s: S[A, Eq], a: A): Boolean
  ...
  def subset(s1: S[A, Eq], s2: S[A, Eq]): Boolean =
    size(s1) <= size(s2) && forall(s1, contains(s2, _))

  def add(s: S[A, Eq], a: A): S[A, Eq]
  def remove(s: S[A, Eq], a: A): S[A, Eq]
  def addAll(s: S[A, Eq], col: Iterable[A]): S[A, Eq] = col.foldLeft(s)(add(_, _))
  def addAll(s: S[A, Eq], it: Iterator[A]): S[A, Eq] = it.foldLeft(s)(add(_, _))
  def removeAll(s: S[A, Eq], col: Iterable[A]): S[A, Eq] = col.foldLeft(s)(remove(_, _))
  def removeAll(s: S[A, Eq], it: Iterator[A]): S[A, Eq] = it.foldLeft(s)(remove(_, _))
  def retain(s: S[A, Eq], p: A => Boolean): S[A, Eq] = foldLeft(s)(s)((s, a) => if(!p(a)) remove(s, a) else s)
  def union(s1: S[A, Eq], s2: S[A, Eq]): S[A, Eq] =
    if(size(s1) > size(s2)) addAll(s1, iterator(s2))
    else                    addAll(s2, iterator(s1))
  def intersect(s1: S[A, Eq], s2: S[A, Eq]): S[A, Eq] =
    if(size(s1) > size(s2)) retain(s2, contains(s1, _))
    else                    retain(s1, contains(s2, _))
  def diff(s1: S[A, Eq], s2: S[A, Eq]): S[A, Eq] =
    if(size(s1) > size(s2)) foldLeft(s2)(s1)(remove(_, _))
    else                    retain(s1, !contains(s2, _))
  def symDiff(s1: S[A, Eq], s2: S[A, Eq]): (S[A, Eq], S[A, Eq]) = (diff(s1, s2), diff(s2, s1))

  def equiv(s1: S[A, Eq], s2: S[A, Eq]): Boolean =
    size(s1) == size(s2) && subset(s1, s2)

  def contentEquivalence: Equiv[S[A, Eq]] = new Equiv[S[A, Eq]] {
...
object Setoid {
  ...
  implicit def arbitrary[S[A0, _ <: Equiv[A0]], A, Eq <: Equiv[A]](implicit S: Setoid[S, A, Eq], A: Arbitrary[A]): Arbitrary[S[A, Eq]] =
    Arbitrary {
  ...
  trait Laws {
    ...
    def unionIdempotence[S[A0, _ <: Equiv[A0]], A, Eq <: Equiv[A]](s: S[A, Eq])(implicit S: Setoid[S, A, Eq]): Boolean =
      S.equiv(S.union(s, s), s)

    def intersectIdempotence[S[A0, _ <: Equiv[A0]], A, Eq <: Equiv[A]](s: S[A, Eq])(implicit S: Setoid[S, A, Eq]): Boolean =
      S.equiv(S.intersect(s, s), s)

    def unionCommutativity[S[A0, _ <: Equiv[A0]], A, Eq <: Equiv[A]](s: S[A, Eq], t: S[A, Eq])(implicit S: Setoid[S, A, Eq]): Boolean =
      S.equiv(S.union(s, t), S.union(t, s))

    def intersectCommutativity[S[A0, _ <: Equiv[A0]], A, Eq <: Equiv[A]](s: S[A, Eq], t: S[A, Eq])(implicit S: Setoid[S, A, Eq]): Boolean =
      S.equiv(S.intersect(s, t), S.intersect(t, s))

    def unionAssociativity[S[A0, _ <: Equiv[A0]], A, Eq <: Equiv[A]](s: S[A, Eq], t: S[A, Eq], u: S[A, Eq])(implicit S: Setoid[S, A, Eq]): Boolean =
      S.equiv(S.union(S.union(s, t), u), S.union(s, S.union(t, u)))

    def intersectAssociativity[S[A0, _ <: Equiv[A0]], A, Eq <: Equiv[A]](s: S[A, Eq], t: S[A, Eq], u: S[A, Eq])(implicit S: Setoid[S, A, Eq]): Boolean =
      S.equiv(S.intersect(S.intersect(s, t), u), S.intersect(s, S.intersect(t, u)))

    def absorption[S[A0, _ <: Equiv[A0]], A, Eq <: Equiv[A]](s: S[A, Eq], t: S[A, Eq])(implicit S: Setoid[S, A, Eq]): Boolean =
      S.equiv(S.intersect(s, S.union(s, t)), s) &&
      S.equiv(S.union(s, S.intersect(s, t)), s)

    def empty[S[A0, _ <: Equiv[A0]], A, Eq <: Equiv[A]](s: S[A, Eq])(implicit S: Setoid[S, A, Eq]): Boolean =
      S.equiv(S.union(s, S.empty), s) &&
      S.equiv(S.intersect(s, S.empty), S.empty)

    def distributivity[S[A0, _ <: Equiv[A0]], A, Eq <: Equiv[A]](s: S[A, Eq], t: S[A, Eq], u: S[A, Eq])(implicit S: Setoid[S, A, Eq]): Boolean =
    ...
    def relativeComplement[S[A0, _ <: Equiv[A0]], A, Eq <: Equiv[A]](s: S[A, Eq], t: S[A, Eq])(implicit S: Setoid[S, A, Eq]): Boolean =
    ...
    def symDiff[S[A0, _ <: Equiv[A0]], A, Eq <: Equiv[A]](s: S[A, Eq], t: S[A, Eq])(implicit S: Setoid[S, A, Eq]): Boolean = {
      val (l, r) = S.symDiff(s, t)
      S.equiv(l, S.diff(s, t)) && S.equiv(r, S.diff(t, s))
    }

    def subset[S[A0, _ <: Equiv[A0]], A, Eq <: Equiv[A]](s: S[A, Eq], t: S[A, Eq])(implicit S: Setoid[S, A, Eq]): Boolean =
      S.subset(s, t) == S.equiv(S.intersect(s, t), s)

    def size[S[A0, _ <: Equiv[A0]], A, Eq <: Equiv[A]](s: S[A, Eq], t: S[A, Eq])(implicit S: Setoid[S, A, Eq]): Boolean =
    ...
    def containsAdded[S[A0, _ <: Equiv[A0]], A, Eq <: Equiv[A]](s: S[A, Eq], a: A)(implicit S: Setoid[S, A, Eq]): Boolean =
      S.contains(S.add(s, a), a)

    def notContainsRemoved[S[A0, _ <: Equiv[A0]], A, Eq <: Equiv[A]](s: S[A, Eq], a: A)(implicit S: Setoid[S, A, Eq]): Boolean =
      !S.contains(S.remove(s, a), a)

    def foldLeft[S[A0, _ <: Equiv[A0]], A, Eq <: Equiv[A]](s: S[A, Eq])(implicit S: Setoid[S, A, Eq]): Boolean = {
    ...
    def containsAll[S[A0, _ <: Equiv[A0]], A, Eq <: Equiv[A]](s: S[A, Eq])(implicit S: Setoid[S, A, Eq], A: Equiv[A]): Boolean =
      S.forall(s, S.contains(s, _))

    def fromIterable[S[A0, _ <: Equiv[A0]], A, Eq <: Equiv[A]](col: Iterable[A])(implicit S: Setoid[S, A, Eq]): Boolean = {
    ...
    def fromIterator[S[A0, _ <: Equiv[A0]], A, Eq <: Equiv[A]](col: Iterable[A])(implicit S: Setoid[S, A, Eq]): Boolean =
      S.equiv(S.fromIterator(col.iterator), S.fromIterable(col))

    def toList[S[A0, _ <: Equiv[A0]], A, Eq <: Equiv[A]](s: S[A, Eq])(implicit S: Setoid[S, A, Eq]): Boolean =
      S.equiv(S.fromIterable(S.toList(s)), s)

    def singleton[S[A0, _ <: Equiv[A0]], A, Eq <: Equiv[A]](a: A)(implicit S: Setoid[S, A, Eq]): Boolean =
      S.equiv(S.singleton(a), S.add(S.empty, a))

    def addAll[S[A0, _ <: Equiv[A0]], A, Eq <: Equiv[A]](s: S[A, Eq], col: Iterable[A])(implicit S: Setoid[S, A, Eq]): Boolean =
      S.equiv(S.addAll(s, col), S.union(s, S.fromIterable(col)))

    def addAllIterator[S[A0, _ <: Equiv[A0]], A, Eq <: Equiv[A]](s: S[A, Eq], col: Iterable[A])(implicit S: Setoid[S, A, Eq]): Boolean =
      S.equiv(S.addAll(s, col.iterator), S.union(s, S.fromIterable(col)))

    def removeAll[S[A0, _ <: Equiv[A0]], A, Eq <: Equiv[A]](s: S[A, Eq], col: Iterable[A])(implicit S: Setoid[S, A, Eq]): Boolean =
      S.equiv(S.removeAll(s, col), S.diff(s, S.fromIterable(col)))

    def removeAllIterator[S[A0, _ <: Equiv[A0]], A, Eq <: Equiv[A]](s: S[A, Eq], col: Iterable[A])(implicit S: Setoid[S, A, Eq]): Boolean =
      S.equiv(S.removeAll(s, col.iterator), S.diff(s, S.fromIterable(col)))

    def retain[S[A0, _ <: Equiv[A0]], A, Eq <: Equiv[A]](s: S[A, Eq], p: A => Boolean)(implicit S: Setoid[S, A, Eq]): Boolean =
      S.equiv(S.retain(s, p), S.fromIterable(S.toList(s).filter(p)))
  }

  def properties[S[A0, _ <: Equiv[A0]], A, Eq <: Equiv[A]](name: String = "Setoid")(implicit S: Setoid[S, A, Eq], EA: Equiv[A], A: Arbitrary[A], PA: Arbitrary[A => Boolean]): Properties =
    new Properties(name) {
      val laws = new Laws {}
