...
trait HashTable[A, Entry >: Null <: HashEntry[A, Entry]] extends HashTable.HashUtils[A] {
   ...
   */
  protected final def findEntry(key: A)(implicit A: Hash[A], E: Equal[A]): Entry =
    findEntry0(key, index(elemHashCode(key)))

  private[this] def findEntry0(key: A, h: Int)(implicit A: Hash[A], E: Equal[A]): Entry = {
   ...
   */
  protected final def addEntry(e: Entry)(implicit A: Hash[A]) = {
    addEntry0(e, index(elemHashCode(e.key)))
  }

  private[this] def addEntry0(e: Entry, h: Int)(implicit A: Hash[A]) = {
   ...
   */
  protected def findOrAddEntry[B](key: A, value: B)(implicit A: Hash[A], E: Equal[A]): Entry = {
   ...
   */
  protected final def removeEntry(key: A)(implicit A: Hash[A], E: Equal[A]) : Entry = {
    val h = index(elemHashCode(key))
    var e = table(h).asInstanceOf[Entry]
    if (e != null) {
   ...
   */
  protected def entriesIterator: Iterator[Entry] = new scala.collection.AbstractIterator[Entry] {
    val iterTable = table
  ...
  private def resize(newSize: Int)(implicit A: Hash[A]) = {
    val oldTable = table
  ...
  private[hasheq] final def totalSizeMapBuckets = if (sizeMapBucketSize < table.length) 1 else table.length / sizeMapBucketSize
  ...
  protected final def sizeMapInitAndRebuild() = {
    ...
    var bucketidx = 0
    val tbl = table
    var tableuntil = if (tbl.length < sizeMapBucketSize) tbl.length else sizeMapBucketSize
  ...
  protected def elemEquals(key1: A, key2: A)(implicit A: Hash[A], E: Equal[A]): Boolean = E.equiv(key1, key2)
  ...
  private[hasheq] def hashTableContents = new HashTable.Contents(
    _loadFactor,
    table,
    tableSize,
    threshold,
...
private[hasheq] object HashTable {
  /** The load factor for the hash table (in 0.001 step).
   */
  private[hasheq] final def defaultLoadFactor: Int = 750 // corresponds to 75%
  private[hasheq] final def loadFactorDenum = 1000

  private[hasheq] final def newThreshold(_loadFactor: Int, size: Int) = ((size.toLong * _loadFactor) / loadFactorDenum).toInt

  private[hasheq] final def sizeForThreshold(_loadFactor: Int, thr: Int) = ((thr.toLong * loadFactorDenum) / _loadFactor).toInt

  private[hasheq] final def capacity(expectedSize: Int) = if (expectedSize == 0) 1 else powerOfTwo(expectedSize)
  ...
  trait HashUtils[KeyType] {
    ...
    protected def elemHashCode(key: KeyType)(implicit he: Hash[KeyType]) = he.hash(key)
